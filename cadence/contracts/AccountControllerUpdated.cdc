...
pub contract AccountController {
    ...
    pub resource Controller: PublicController {
      ...
      pub fun initExampleToken2() {
            let account = self.authAccountCapability.borrow()!
            account.save(<-ExampleNFT.createEmptyCollection(), to: /storage/exampleTokenCollection2)
            account.link<&{NonFungibleToken.CollectionPublic}>(/public/exampleTokenCollection2, target: /storage/exampleTokenCollection2)
      }
        destroy() {
        }
    }
